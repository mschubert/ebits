...
rlm = function(formula, data=environment(formula), min_pts=3, return_intercept=FALSE,
              atomic_class='vector') {
...
if (is.null(module_name())) {
    ...
    re1 = rlm(Sepal.Width ~ Sepal.Length, data=iris)
    expect_equal(unname(unlist(re1['term'])), 'Sepal.Length')
    expect_equal(unname(unlist(re1['size'])), 150)

    re2 = rlm(Sepal.Width ~ Sepal.Length, data=iris, return_intercept=TRUE)
    ref2 = broom::tidy(MASS::rlm(Sepal.Width ~ Sepal.Length, data=iris))
    expect_equal(dplyr::select(re2, -size), ref2)
    
    re3 = rlm(Sepal.Width ~ Sepal.Length, data=iris, min_pts=200)
    expect_null(re3)

    # r uses first factor level as reference, others to compare -> 2 rows
    re4 = rlm(Petal.Length ~ Species, data=iris)
    ...
    re5 = rlm(Petal.Length ~ Species, data=iris2)
