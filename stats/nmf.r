...
.nmf = function(A, k, max_iter, seed=1234, tolerance=1e-4, num_clusterings=10) {
    one_iter = function(seed) {
        ...
        list(W = matrix(W, nrow=m, ncol=k, dimnames=list(rownames(A), NULL)),
             H = matrix(H, nrow=k, ncol=n, dimnames=list(NULL, colnames(A))),
             iter=result$maxiter)
    }
...
nmf = function(A, k, max_iter=1000, seed=1234, tolerance=1e-4, rep=10, hpc_args=NULL) {
    # construct df here, call above func
    result = .df$call(data.frame(k = k),
                      .nmf,
