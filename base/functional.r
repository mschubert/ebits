...
#' Argument matching with defaults
#'
#' \code{eval_call} takes an unevaluated \code{call} object and evaluates all
#' its argument, as well as the function name, to produce a standalone
#' \code{call} object that no longer relies on its current context.
#' @param envir the environment to evaluate the call in
#' @return \code{eval_call} returns a \code{call} object with its arguments
#' evaluated.
#' @examples
#' # Show the difference between `match_call_defaults` and `eval_call`
#' f = function (x) match_call_defaults()
#' g = function (x) eval_call()
#' f(1 + 2) # => f(x = 1 + 2)
#' g(1 + 2) # => (function (x) eval_call())(x = 3)
